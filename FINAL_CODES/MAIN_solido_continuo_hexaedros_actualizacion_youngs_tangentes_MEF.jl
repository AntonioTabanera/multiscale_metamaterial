...
numero_fichero="FLEX_2" # "FLEX_25"

#generar_tabla_tension_deformacion_Osgood()
fichero_ensayo=open("C:\\__UNIVERSIDAD__\\AEROESPACIAL\\4º GIA\\TFG\\COMPARACIONES\\comparaciones_solido_fuerza_$(lpad(numero_fichero,2,"0")).txt","w") #crear el archivo para recoger datos del ensayo
fichero_ensayo_2=open("C:\\__UNIVERSIDAD__\\AEROESPACIAL\\4º GIA\\TFG\\COMPARACIONES\\comparaciones_solido_energ_total_$(lpad(numero_fichero,2,"0")).txt","w") #crear el archivo para recoger datos del ensayo
fichero_ensayo_3=open("C:\\__UNIVERSIDAD__\\AEROESPACIAL\\4º GIA\\TFG\\COMPARACIONES\\comparaciones_solido_inc_energ_total_$(lpad(numero_fichero,2,"0")).txt","w") #crear el archivo para recoger datos del ensayo
fichero_ensayo_4=open("C:\\__UNIVERSIDAD__\\AEROESPACIAL\\4º GIA\\TFG\\COMPARACIONES\\comparaciones_solido_energ_hexaedros_$(lpad(numero_fichero,2,"0")).txt","w") #crear el archivo para recoger datos del ensayo
fichero_ensayo_5=open("C:\\__UNIVERSIDAD__\\AEROESPACIAL\\4º GIA\\TFG\\COMPARACIONES\\comparaciones_solido_inc_energ_hexaedros_$(lpad(numero_fichero,2,"0")).txt","w") #crear el archivo para recoger datos del ensayo
...
### HIPERPARAMETROS
iter_max=10 #Numero de iteraciones hasta alcanzar el valor del desplazamiento final
...
# restriccion 1
punto_restriccion=[0 "all" "all"]#[0 "all" "all"]
gdl_restriccion=[1 1 1]#[1 0 0]#
...
valor_desplazamiento=[ 0  0 10e-1]
...
energia_acumulada=0
energias_hexaedros_acumulado=zeros(N_elementos); f_acumulado_hexaedros=zeros(24, N_elementos)
...
for iter_desplazamiento in 1:iter_max
    ...
    global u_solido, f_solido, f_elementos, energia_acumulada, epsilon_voigt_acumulado, deform_equivalente, inc_deform_equivalente, inc_energias_elementos, energias_hexaedros_acumulado, f_acumulado_hexaedros, Youngs_tangentes_elementos, fuerza_reaccion_x_iters, fuerza_reaccion_y_iters, fuerza_reaccion_z_iters, desplaz_iters, fichero_ensayo, fichero_ensayo_2, fichero_ensayo_3, fichero_ensayo_4, fichero_ensayo_5, N_elementos
    ...
    #inc_u_solido, inc_f_solido, inc_energias_elementos, deform_equivalente, inc_deform_equivalente, epsilon_voigt_acumulado=resolucion_solido_3D(coords_3D, conectividad_3D, Youngs_elementos, epsilon_voigt_acumulado, gdl_restring_globales_solido, valor_restricciones_globales_solido)
    ...
    inc_u_solido, inc_f_solido, inc_energias_elementos, invariante_deform, inc_def_equivalente, epsilon_voigt_acumulado, Youngs_tangentes_elementos, energias_hexaedros_acumulado, f_acumulado_hexaedros =  resolucion_iterativa_solido(coords_3D, conectividad_3D, Youngs_tangentes_elementos, epsilon_voigt_acumulado, gdl_restring_globales_solido, valor_restricciones_globales_solido, f_acumulado_hexaedros, energias_hexaedros_acumulado)
    ...
    desplaz=maximum(valor_restricciones_globales_solido)*iter_desplazamiento

    
    push!(fuerza_reaccion_x_iters,abs(fuerza_x_total))
    push!(fuerza_reaccion_y_iters,abs(fuerza_y_total))
    push!(fuerza_reaccion_z_iters,abs(fuerza_z_total))
    ...
    fichero_ensayo=open("C:\\__UNIVERSIDAD__\\AEROESPACIAL\\4º GIA\\TFG\\COMPARACIONES\\comparaciones_solido_fuerza_$(lpad(numero_fichero,2,"0")).txt","a") 
    fichero_ensayo_2=open("C:\\__UNIVERSIDAD__\\AEROESPACIAL\\4º GIA\\TFG\\COMPARACIONES\\comparaciones_solido_energ_total_$(lpad(numero_fichero,2,"0")).txt","a") 
    fichero_ensayo_3=open("C:\\__UNIVERSIDAD__\\AEROESPACIAL\\4º GIA\\TFG\\COMPARACIONES\\comparaciones_solido_inc_energ_total_$(lpad(numero_fichero,2,"0")).txt","a") 
    fichero_ensayo_4=open("C:\\__UNIVERSIDAD__\\AEROESPACIAL\\4º GIA\\TFG\\COMPARACIONES\\comparaciones_solido_energ_hexaedros_$(lpad(numero_fichero,2,"0")).txt","a") 
    fichero_ensayo_5=open("C:\\__UNIVERSIDAD__\\AEROESPACIAL\\4º GIA\\TFG\\COMPARACIONES\\comparaciones_solido_inc_energ_hexaedros_$(lpad(numero_fichero,2,"0")).txt","a") 

    ### GENERAR FICHEROS CON LOS VALORES 
    println(fichero_ensayo, fuerza_reaccion_z_iters[end]," ",desplaz_iters[end] )
    println(fichero_ensayo_2, sum(energias_hexaedros_acumulado))
    println(fichero_ensayo_3, sum(inc_energias_elementos))

    for i in 1:N_elementos
        println(fichero_ensayo_4, energias_hexaedros_acumulado[i])
        println(fichero_ensayo_5, inc_energias_elementos[i])
    end
