...
plot_paraview=false   #Para generar o no los ficheros de paraview para visualizar el problema

#generar_tabla_tension_deformacion_Osgood()
fichero_ensayo=open("multiescala_energ_element_$(lpad(numero_fichero,2,"0")).txt","w") #crear el archivo para recoger datos del ensayo
fichero_ensayo_2=open("multiescala_inc_energ_element_$(lpad(numero_fichero,2,"0")).txt","w") #crear el archivo para recoger datos del ensayo
fichero_ensayo_3=open("multiescala_energ_total_$(lpad(numero_fichero,2,"0")).txt","w") #crear el archivo para recoger datos del ensayo
fichero_ensayo_4=open("multiescala_inc_energ_total_$(lpad(numero_fichero,2,"0")).txt","w") #crear el archivo para recoger datos del ensayo
fichero_ensayo_5=open("multiescala_fuerza_$(lpad(numero_fichero,2,"0")).txt","w") #crear el archivo para recoger datos del ensayo
fichero_ensayo_6=open("multiescala_energ_zona_metam_$(lpad(numero_fichero,2,"0")).txt","w") #crear el archivo para recoger datos del ensayo
fichero_ensayo_7=open("multiescala_inc_energ_zona_metam_$(lpad(numero_fichero,2,"0")).txt","w") #crear el archivo para recoger datos del ensayo
fichero_ensayo_8=open("multiescala_energ_metam_total_$(lpad(numero_fichero,2,"0")).txt","w") #crear el archivo para recoger datos del ensayo
fichero_ensayo_9=open("multiescala_inc_energ_metam_total_$(lpad(numero_fichero,2,"0")).txt","w") #crear el archivo para recoger datos del ensayo
fichero_ensayo_10=open("multiescala_curva_tension_deformacion_hexaedros_gruesos_$(lpad(numero_fichero,2,"0")).txt","w") #crear el archivo para recoger datos del ensayo
fichero_ensayo_11=open("multiescala_deformacion_barra_max_hexaedros_gruesos_$(lpad(numero_fichero,2,"0")).txt","w") #crear el archivo para recoger datos del ensayo



### DIMENSIONES PROBETA RECTANGULAR Y CELDILLAS METAMATERIAL (PONER VALORES)
...
### TIPO DE CELDILLA UNIDAD PARA EL METAMATERIAL
tipo_celdilla="celdilla_FCC"    #"celdilla_simple"


### HIPERPARAMETROS (MODIFICAR SEGUN SE QUIERAN)
gradiente=true#       #Para aplicar o no el método del gradiente para redcir errores al hacer el acoplamiento multiescala

iter_desplaz=100;        #Numero de iteraciones en desplazamiento hasta alcanzar el valor del desplazamiento final
...
exponente_reparto_young=1   #Si el exponente es cero, entonces, se les pone a todos los hexaedros finos el mismo Young que al hexaedro grueso

# HIPERPARAMETROS ADICIONALES PARA EL METODO DEL GRADIENTE
step_gradiente_desplaz=0.01*10/(194000*radio^2)         #Constante para definir el nuevo valor actualizado de desplazamiento en el metodo de gradiente
step_gradiente_giros=0.01/(10^2)*10^3/(194000*radio^4)  #Constante para definir el nuevo valor actualizado de giro en el metodo de gradiente
iter_max_solido_grad=1      #Numero de iteraciones para converger en cada desplazamiento (sustituye a iter_max)
iter_max_gradiente=10        #Numero de iteraciones maximas para cada vez que se intenta corregir con el metodo de gradiente
...
########################### DISTINTAS RESTRICCIONES (DESCOMENTAR O AÑADIR LAS QUE SE QUIERAN)
...
########################### DISTINTOS TIPOS DE CARGA (DESCOMENTAR Y AÑADIR LOS QUE SE QUIERAN)
...
# desplazamiento 1
punto_desplazamiento=[X "all" "all"]#
valor_desplazamiento=[1.5e-0  0  0 ]
...
if tipo_celdilla=="celdilla_FCC"
    coords_metam,conectividad_metam=celdilla_FCC(X,Y,Z,Nx,Ny,Nz)
elseif tipo_celdilla=="celdilla_simple"
    coords_metam,conectividad_metam=celdilla_simple(X,Y,Z,Nx,Ny,Nz)
else
    println("Tipo de celdilla no especificado. Celdilla simple por defecto")
    coords_metam,conectividad_metam=celdilla_simple(X,Y,Z,Nx,Ny,Nz)
end
...
### NOMBRES A LOS FICHEROS
println(fichero_ensayo,   "ENERGIA ACUMULADA EN LOS ELEMENTOS SOLIDOS GRUESOS CALCULADOS CON MODELO DE SOLIDO HOMOGENEO");println(fichero_ensayo," ");close(fichero_ensayo)
println(fichero_ensayo_2, "INCREMENTO DE ENERGIA EN CADA STEP DE LOS ELEMENTOS SOLIDOS GRUESOS CALCULADOS CON MODELO DE SOLIDO HOMOGENEO") ;println(fichero_ensayo_2," ");close(fichero_ensayo_2)    
println(fichero_ensayo_3, "ENERGIA ACUMULADA EN LA ESTRUCTURA COMPLETA CALCULADA CON MODELO DE SOLIDO HOMOGENEO");println(fichero_ensayo_3," ");close(fichero_ensayo_3)
println(fichero_ensayo_4, "INCREMENTO DE ENERGIA EN CADA STEP DE LA ESTRUCTURA COMPLETA CALCULADA CON MODELO DE SOLIDO HOMOGENEO");println(fichero_ensayo_4," ");close(fichero_ensayo_4)
println(fichero_ensayo_5, "FUERZA TOTAL EXTERIOR APLICADA");println(fichero_ensayo_5," ");close(fichero_ensayo_5)
println(fichero_ensayo_6, "ENERGIA ACUMULADA EN LOS ELEMENTOS SOLIDOS GRUESOS CALCULADOS CON REGIONES DE METAMATERIAL");println(fichero_ensayo_6," ");close(fichero_ensayo_6)
println(fichero_ensayo_7, "INCREMENTO DE ENERGIA EN CADA STEP DE LOS ELEMENTOS SOLIDOS GRUESOS CALCULADOS CON REGIONES DE METAMATERIAL");println(fichero_ensayo_7," ");close(fichero_ensayo_7)
println(fichero_ensayo_8, "ENERGIA ACUMULADA EN LA ESTRUCTURA COMPLETA CALCULADA CON REGIONES DE METAMATERIAL");println(fichero_ensayo_8," ");close(fichero_ensayo_8)
println(fichero_ensayo_9, "INCREMENTO DE ENERGIA EN CADA STEP DE LA ESTRUCTURA COMPLETA CALCULADA CON REGIONES DE METAMATERIAL");println(fichero_ensayo_9," ");close(fichero_ensayo_9)
println(fichero_ensayo_11, "DEFORMACION MAXIMA (EN UNA BARRA) EN LAS DISTINTAS REGIONES PARA CADA STEP");println(fichero_ensayo_11," ");close(fichero_ensayo_11)


for iter_desplazamiento in 1:iter_desplaz

    println();println("##############  STEP EN DESPLAZAMIENTO ",iter_desplazamiento,"/",iter_desplaz,"  ##############");println()
    ...
    #println("ENERGIA ACUMULADA SOLIDO COMPLETO= ",sum(energias_hexaedros_acumulado));println();
    ...
    fichero_ensayo=open("multiescala_energ_element_$(lpad(numero_fichero,2,"0")).txt","a") 
    fichero_ensayo_2=open("multiescala_inc_energ_element_$(lpad(numero_fichero,2,"0")).txt","a") 
    fichero_ensayo_3=open("multiescala_energ_total_$(lpad(numero_fichero,2,"0")).txt","a") 
    fichero_ensayo_4=open("multiescala_inc_energ_total_$(lpad(numero_fichero,2,"0")).txt","a") 
    fichero_ensayo_6=open("multiescala_energ_zona_metam_$(lpad(numero_fichero,2,"0")).txt","a") 
    fichero_ensayo_5=open("multiescala_fuerza_$(lpad(numero_fichero,2,"0")).txt","a") 
    fichero_ensayo_7=open("multiescala_inc_energ_zona_metam_$(lpad(numero_fichero,2,"0")).txt","a") 
    fichero_ensayo_8=open("multiescala_energ_metam_total_$(lpad(numero_fichero,2,"0")).txt","a")
    fichero_ensayo_9=open("multiescala_inc_energ_metam_total_$(lpad(numero_fichero,2,"0")).txt","a")
    fichero_ensayo_10=open("multiescala_curva_tension_deformacion_hexaedros_gruesos_$(lpad(numero_fichero,2,"0")).txt","a")
    fichero_ensayo_11=open("multiescala_deformacion_barra_max_hexaedros_gruesos_$(lpad(numero_fichero,2,"0")).txt","a")  

    println(fichero_ensayo," "); println(fichero_ensayo,   "##############  STEP EN DESPLAZAMIENTO ",iter_desplazamiento,"/",iter_desplaz,"  ##############")
    println(fichero_ensayo_2," "); println(fichero_ensayo_2, "##############  STEP EN DESPLAZAMIENTO ",iter_desplazamiento,"/",iter_desplaz,"  ##############")     
    println(fichero_ensayo_5," "); println(fichero_ensayo_5, "##############  STEP EN DESPLAZAMIENTO ",iter_desplazamiento,"/",iter_desplaz,"  ##############")
    println(fichero_ensayo_6," "); println(fichero_ensayo_6, "##############  STEP EN DESPLAZAMIENTO ",iter_desplazamiento,"/",iter_desplaz,"  ##############")
    println(fichero_ensayo_9," "); println(fichero_ensayo_9, "##############  STEP EN DESPLAZAMIENTO ",iter_desplazamiento,"/",iter_desplaz,"  ##############")
    println(fichero_ensayo_11," "); println(fichero_ensayo_11,"##############  STEP EN DESPLAZAMIENTO ",iter_desplazamiento,"/",iter_desplaz,"  ##############")

    for i in 1:N_hexaedros_gruesos
        println(fichero_ensayo, "Region ", i," ", energias_solidos_gruesos[i])
        println(fichero_ensayo_2, "Region ", i," ",inc_energias_solidos_gruesos[i])
        println(fichero_ensayo_6, "Region ", i," ",energia_elemento_solido_grueso_metam[i])
        println(fichero_ensayo_7, "Region ", i," ",inc_energia_elemento_solido_grueso_metam[i])       
        println(fichero_ensayo_11, "Region ", i," ",maximum(values(invariante_def_memoria_metam[i])))
    end
    println(fichero_ensayo_3, "STEP ",iter_desplazamiento,"/",iter_desplaz," " ,sum(energias_solidos_gruesos))
    println(fichero_ensayo_4, "STEP ",iter_desplazamiento,"/",iter_desplaz," " ,sum(inc_energias_solidos_gruesos))
    println(fichero_ensayo_8, "STEP ",iter_desplazamiento,"/",iter_desplaz," " ,sum(values(energia_elemento_solido_grueso_metam)))
    println(fichero_ensayo_9, "STEP ",iter_desplazamiento,"/",iter_desplaz," " ,sum(values(inc_energia_elemento_solido_grueso_metam)))

    println(fichero_ensayo_5, "STEP ",iter_desplazamiento,"/",iter_desplaz," " ,fuerza_reaccion_x_iters[end]," ",desplaz_iters[end])
